...
function getDernierMois($pdo, $idMembre)
{
		$req = "select max(mois) as dernierMois from fichefrais where idMembre = '$idMembre'";
		$res = $pdo->query($req);
		$laLigne = $res->fetch();
...
function creationFichesFrais($pdo)
{
	...
	foreach($lesMembres as $unMembre)
	{
		...
		while($moisCourant >= $moisDebut)
		{
			...
			values ('$idMembre','$moisCourant',$nbJustificatifs,0,'$dateModif','$etat');";
			$pdo->exec($req);
...
function creationFraisHorsForfait($pdo)
{
	...
	foreach($lesFichesFrais as $uneFicheFrais)
	{
		...
		for($i=0;$i<=$nbFrais;$i++)
		{
			...
			$req = "insert into lignefraishorsforfait(idMembre,mois,libelle,date,montant)
			values('$idMembre','$mois','$lib','$hasardMois',$hasardMontant);";
			$pdo->exec($req);
		}
	}
}
...
function majFicheFrais($pdo)
{
	...
	foreach($lesFichesFrais as $uneFicheFrais)
	{
		$idMembre = $uneFicheFrais['idMembre'];
		$mois =  $uneFicheFrais['mois'];
		$dernierMois = getDernierMois($pdo, $idMembre);
		$req = "select sum(montant) as cumul from ligneFraisHorsForfait where ligneFraisHorsForfait.idMembre = '$idMembre' 
		...
		ligneFraisForfait.idFraisForfait = fraisForfait.id   and   ligneFraisForfait.idMembre = '$idMembre' 
		...
		idMembre = '$idMembre' and mois='$mois'";
		$pdo->exec($req);
